...
function checkBeforeSubmit(username) {
    ...
    xhr.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            ...
            if (response.exists) {
                document.getElementById("input-error").innerHTML = "This username already exists!";
            } else if(username.trim() !== "" && password.trim() !== "" && emailRegex.test(email) ) { //an exei sumplhrwsei ola ta pedia
                reg_openPopup(); //anoikse to pop up
                document.getElementById("input-error").innerHTML = "&nbsp";
            }
        }
    };
...
//kathusterhsh gia elegxo enw plhktrologei, meta apo 500 ms afou stamathsei na plhktrologei elegkse
...
// Kane to sumbit otan pathsei ok sto pop up
function submitForm() {
    var popup = document.getElementById("popup"); 
...
//an egine to registration kane redirect
